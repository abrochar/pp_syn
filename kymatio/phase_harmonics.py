...
class PhaseHarmonics2d(object):
    def __init__(self, M, N, J, L, delta_j, delta_l, delta_k, delta_n, nb_chunks, chunk_id, devid=0):
    ...
    def filters_tensor(self):
        ...
        if self.dn>0:
            ell = torch.arange(L2).type(torch.float)
            thetas = np.pi/2 - ell*2*np.pi/L2
            dirs = torch.stack((torch.sin(thetas), torch.cos(thetas)), dim=-1).unsqueeze(0)
            scales = torch.arange(J).type(torch.float)
            scales = 2**(scales).unsqueeze(-1)
            shifts = torch.stack((torch.matmul(scales, dirs[...,0]), torch.matmul(scales, dirs[...,1])), dim=-1)

            self.hatpsi_shift = shift_filt(self.hatpsi, shifts)

            self.hatpsi = torch.stack((self.hatpsi, self.hatpsi_shift), dim=2).reshape(1, J*L2*2, self.N, self.N, 2).contiguous()
 ...
 #           assert(nb==1 and nc==1) # for submeanC
            ...
            for idxb in range(nb):
                for idxc in range(nc):
                    ...
                    xpsi_bc_flat = xpsi_bc.view(1,J*L2*(dn+1),M,N,2)
                    # select la1, et la2, P_c = number of |la1| in this chunk
                    xpsi_bc_la1 = xpsi_bc_flat[:,self.this_wph['la1'],...]
                    xpsi_bc_la2 = xpsi_bc_flat[:,self.this_wph['la2'],...]
                    # phase harmonics
                    k1 = self.this_wph['k1']
                    k2 = self.this_wph['k2']
            ...
            Sout = input.new(nb, nc, 1, \
                             1, 1, 2)
            Sout[:,:,0,:,:,:] = torch.mean(torch.mean(xpsi0_mod2,-2,True),-3,True)*1e1
...
#                    xpsi_bc_la2 = torch.index_select(xpsi_bc, 1, self.this_wph['la2']) # (1,P_c,M,N,2)
                    xpsi_bc_la1 = xpsi_bc[:,self.this_wph['la1'],...]
                    xpsi_bc_la2 = xpsi_bc[:,self.this_wph['la2'],...]
